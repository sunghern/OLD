...
class BasicBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        #zero_value = torch.nonzero(out)
            ...
            #zero_before = zero_value.reshape(-1)
            #print("first shape")
            #print(zero_before.shape)
            #zero_count = torch.nonzero(out)
            #zero_after = zero_count.reshape(-1)

            #print(1-torch.numel(zero_after)/torch.numel(zero_before))

            #out_check = torch.where(out_check == 0,1,0)
            #zero_dim = out_check.sum(dim=1).squeeze()
        ...
        #zero_value_t = torch.nonzero(out)
            ...
            #out = out * self.channel_pruning(out)
            #zero_before = zero_value.reshape(-1)
            #zero_before_t = zero_value_t.reshape(-1)
            #print(zero_before.shape)
            #zero_count = torch.nonzero(out)
            #zero_after = zero_count.reshape(-1)

            #print(1-torch.numel(zero_after)/torch.numel(zero_before))
            #print(1-torch.numel(zero_after)/torch.numel(zero_before_t))
            #out_check = out.sum(dim = (2,3))
            #out_check = torch.where(out_check == 0,1,0)
            #zero_dim = out_check.sum(dim=1).squeeze()
            #total_dim = out.shape[1]
            #print("percentage 2")
        ...
        out = F.relu(out)
        #return out
        if partition_point == True and self.pruning != 1:
